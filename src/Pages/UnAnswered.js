...
class UnAnswered extends Component {
    ...
    componentDidMount() {
        document.title = "QnA | UnAnswered"
        this.getUnAnsweredQuestion();
    }

    getUnAnsweredQuestion() {
        fetch(url + "getUnAnsweredQuestion/" + this.state.perPage + "/" + this.state.offset)
            .then(res => {
                return res.json();
            })
            .then(result => {
    ...
    render() {
        // let checkQuestions = this.state.questions;
        // if (checkQuestions.length === 0) {
        //     return <React.Fragment><NoResult />
        //         <h2 className="text-center">Oops, Every questions are answered</h2>
        //     </React.Fragment>
        // }
